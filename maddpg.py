...
class MultiAgent():
    ...
    def learn_from_replay(self,):
        ...
        if len(self.memory) > BATCH_SIZE:
            experiences = self.memory.sample()
            self.learn1(experiences, GAMMA)
            self.learn2(experiences, GAMMA)


    def act1(self, state, add_noise=True):
    ...
    def act2(self, state, add_noise=True):
        ...
        with torch.no_grad():
            action = self.actor_local2(state).cpu().data.numpy()
    ...
    def learn2(self, experiences, gamma):
        ...
        # ---------------------------- update critic ---------------------------- #
        # Get predicted next-state actions and Q values from target models
        actions_next = self.actor_target2(next_states)
        Q_targets_next = self.critic_target2(next_states, actions_next)
        # Compute Q targets for current states (y_i)
        Q_targets = rewards + (gamma * Q_targets_next * (1 - dones))
        # Compute critic loss
        Q_expected = self.critic_local2(states, actions)
        critic_loss = F.mse_loss(Q_expected, Q_targets)
        # Minimize the loss
        ...
        # ---------------------------- update actor ---------------------------- #
        # Compute actor loss
        actions_pred = self.actor_local2(states)
        actor_loss = -self.critic_local2(states, actions_pred).mean()
        # Minimize the loss
